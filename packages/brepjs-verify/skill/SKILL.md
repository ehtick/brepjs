...
- **`fillet`/`chamfer`/`shell`/`offset` need a `ValidSolid`.** Primitives (`box`/`cylinder`/`cone`) already are one, and booleans preserve it — a `cut`/`fuse` rooted at a `ValidSolid` stays valid — so a primitive-rooted chain feeds them directly. But a shape born from a **2D-sketch `.extrude()`/`.revolve()`** (or `loft`/`sweep`) is typed `Shape3D`; passing that union to these ops — or to `validSolid` itself, whose param is a concrete `Solid` — is `TS2345: not assignable to Shapeable<ValidSolid>`. Lift it in two steps (narrow, then prove): `if (!isSolid(x)) throw …; const solid = unwrap(validSolid(x));`. Or sidestep it — build the prism from a primitive when you know you'll `fillet`/`shell` it. (See `references/modifiers.md`.)
- **Select edges/faces; don't fillet/chamfer everything.** `fillet(solid, radius)` (no edge list) rounds EVERY edge and frequently fails (`FILLET_FAILED`). Pass an edge list: `edgeFinder().inDirection('Z').findAll(solid)`. Note `inDirection` matches BOTH ± orientations; discriminate a single face/edge by position with `.when(f => getBounds(f).zMax > t)` — `getBounds` is its own import from `'brepjs'`. Finders take a **direction** (`'X'`/`'Y'`/`'Z'`/a `Vec3`), never a plane: a top face is `faceFinder().inDirection('Z')` or `.parallelTo('Z')`; **`parallelTo('XY')` fails `--check`** (`TS2345`) — a face parallel to the XY plane has a `Z` normal, so pass `'Z'`. (See `references/modifiers.md`.)
