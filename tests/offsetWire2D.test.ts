...
import { box, getFaces, getWires, offsetWire2D, isOk, unwrap, curveLength } from '@/index.js';

beforeAll(async () => {
  await initKernel();
}, 30000);

describe.skipIf(shouldSkipSuite('offsetWire2D.chamferJoin'))(
  'OCCT-specific: offsetWire2D chamfer join',
  () => {
