...
describe('adaptedCurveToPathElem', () => {
  describe('LINE', () => {
    it('produces an L command for a line segment', () => {
      ...
      const result = adaptedCurveToPathElem(adaptor, curve.lastPoint);
  ...
  describe('CIRCLE', () => {
    ...
    it('produces an A command for a full circle (360 degree nudge)', () => {
      ...
      // Full circles are split by approximateAsSvgCompatibleCurve, but we
      // can still test the raw adaptor path. For a full circle the first
      // and last points coincide — the function nudges the endpoint.
      const curves = approximateAsSvgCompatibleCurve([circle]);
      // Full circle should be split into 2 arcs
      expect(curves.length).toBe(2);
      for (const c of curves) {
        const a = r(c.adaptor());
        const result = adaptedCurveToPathElem(a, c.lastPoint);
        expect(result).toMatch(/^A\s/);
      }
    });
  });
